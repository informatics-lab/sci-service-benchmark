...
try:
    ...
    log('Arg 1 must be a month range 1-12.')
...
try:
    ...
    log('Arg 2 must be a refion A, B, C or D.')
...
try:
    ...
    log('Expect two args, fist a integer month (1-12) second region A, B, C or D.')
...
def average_for_month_and_region(month, region, stash, files):
    ...
    for level, stats in data_levels.items():
        log('for level %s we have %s slices' % (level, stats['count']))
...
log("Done in %s"  % ( end_time - start_time))
